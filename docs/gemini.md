...
The shared `OnDeviceIntelligence` boundary uses compact local image heuristics, Android's local face detector, and ZXing QR decoding. Latin OCR is an explicit local capability-unavailable/permanent failure until a compact model fits the hard gates; it never downloads or lies about a processed result. Keep every generated APK <=30 MB and the release AAB <=20 MB.

## Release truth and workflow

`app/build.gradle.kts` is authoritative for checked-in metadata: `versionCode = 15`, `versionName = "2.0.8"`, `targetSdk = 36`. Play Console state is external and must be verified at upload time, not documented as fact. `finish_release.sh` only builds and verifies; it does not push, open browsers, or upload.
